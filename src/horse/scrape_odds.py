...
if not path.isdir('src/horse/logs'):
    mkdir('src/horse/logs')
logging.basicConfig(filename=f'src/horse/logs/{now}.log', encoding='utf-8', level=logging.INFO)
...
def scrape_odds(hidden=False):
    ...
    # Getting module list.
    site_paths = glob(path.join("src", "horse", "modules", "*.py"))
    site_list = [path.basename(site_path)[0:-3] for site_path in site_paths]
    modules = [getattr(import_module("src.horse.modules." + module), module) for module in site_list]

    # Clearing data files
    if path.isdir('src/horse/data'):
        rmtree('src/horse/data')
    mkdir('src/horse/data')
