...
from src.webscraper.modules import Betr, Pinnacle, Pointsbet, Sportsbet, Betfairlay

def main(source="Betr", amount=50):
    Betr.Betr().write_to_csv()
    Betfairlay.Betfairlay().write_to_csv()
    # Sportsbet.Sportsbet().write_to_csv()
    # Pointsbet.Pointsbet().write_to_csv()
    # Pinnacle.Pinnacle().write_to_csv()
    pinnacle_df = pd.read_csv(f"src/webscraper/data/Betfairlay.csv")

    pinnacle_df["Lay 1"] = pinnacle_df["Odds 1"]
    pinnacle_df["Lay 2"] = pinnacle_df["Odds 2"]
