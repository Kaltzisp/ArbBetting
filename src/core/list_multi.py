...
def main(source="Betr", amount=50):
    scrape_odds(site_list=[source, "Pinnacle"])
    ...
    verify_df = opp_df[["Team 1", "Team 2", "Odds 1", "Odds 2"]].merge(pinnacle_df[["Team 1", "Team 2", "Odds 1", "Odds 2"]], on=["Team 1", "Team 2"], suffixes=(f"_{source}", "_Pinnacle"))
    verify_df["Correct"] = (verify_df["Odds 1_Pinnacle"] > verify_df[f"Odds 1_{source}"]) & (verify_df["Odds 2_Pinnacle"] > verify_df[f"Odds 2_{source}"])
    if not (verify_df["Correct"].eq(True).all()):
        print("WARNING: Potential Misprice")


    opp_df = opp_df[["Odds 1", "Odds 2", "Team 1", "Team 2", "Game"]].merge(pinnacle_df[["Probability 1", "Probability 2", "Team 1", "Team 2", "Game"]], on=["Team 1", "Team 2", "Game"])
